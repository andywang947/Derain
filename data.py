...
class RainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        addrain_img = Image.open(os.path.join(self.image_dir, 'input', self.image_list[idx])).convert("RGB") # notice, 現在是沒有加雨的版本!
        label = Image.open(os.path.join(self.image_dir, 'input', self.label_list[idx])).convert("RGB") # notice, 這個 label 就是 input, 因為有些 dataset 沒有 ground truth.

        image = F.to_tensor(image)
        addrain_img = F.to_tensor(addrain_img)
        label = F.to_tensor(label)
        
        name = self.image_list[idx]
        return image, addrain_img, label, name
